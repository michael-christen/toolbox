...
graphviz_layout = nx.nx_agraph.graphviz_layout(G, prog="dot", args="-Gnodesep=0.2 -Granksep=0.4")
...
graph_renderer.node_renderer.selection_glyph = Circle(radius=1, fill_color="orange")
...
def update_graph_colors(selected_node):
    ...
    node_renderer.data["fill_color"] = ["skyblue"] * len(node_renderer.data["index"])
    edge_renderer.data["line_color"] = ["gray"] * len(edge_renderer.data["start"])
    ...
    for idx, (start, end) in enumerate(zip(edge_renderer.data["start"], edge_renderer.data["end"])):
...
def toggle_subgraph(active):
    ...
    if active:
        ...
        layout_subgraph = nx.nx_agraph.graphviz_layout(subgraph, prog="dot", args="-Gnodesep=0.2 -Granksep=0.4")
