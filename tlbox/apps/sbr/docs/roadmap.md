...
Each peripheral needs a C++ driver and a software simulator so the control stack
can be tested on a host without hardware.
...
The RPC service is the interface between firmware and host tooling. Expand it to
cover all sensors and actuators so the robot can be commanded and observed over
the wire.
...
Once hardware arrives, work through systematic bringup before running the full
control stack.
