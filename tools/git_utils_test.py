...
class TestGitUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        subprocess.check_call(['git', 'init', '--initial-branch=main'], cwd=self.tmp_path)
    ...
    def _cmd(self, args: list[str | pathlib.Path]) -> str:
        return subprocess.check_output(args, cwd=self.tmp_path
                                       ).decode('utf-8').strip()
    ...
    def test_git_utils(self):
        ...
        self.assertEqual(git_utils.get_files_changed_at_commit(commit=test_file_commit_hash,
                                                               git_directory=self.tmp_path),
                         [test_file.relative_to(self.tmp_path)])
        ...
        expected_list = git_utils.FileCommitMap(
            commit_map={
                ...
                mv_commit_hash: set([pathlib.Path('test_file.txt'), pathlib.Path('moved_file.txt')]),
            },
            file_map={
