...
def _show_exclusions(pattern: str,
...
def refine_dataframe(df: pandas.DataFrame,
    ...
    for pattern, exclusion in zip(refinement.name_patterns, exclude_by_name):
        _show_exclusions(pattern=pattern, exclusion=exclusion, df=df, verbosity=verbosity)
...
def full_refinement(graph: networkx.DiGraph,
                    df: pandas.DataFrame,
                    refinement: RefinementConfig,
                    verbosity: Verbosity) -> pandas.DataFrame:
